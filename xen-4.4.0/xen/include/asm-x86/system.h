...
/* 
 *  pushf : pushes the current flags onto the stack. (puhfq: 64-bit mode)
 *  popq : pop the flags from the stack into register. (popq : 64-bit mode)
 *  local_save_flags(x) : 현재 상태 플래그를 변수 x에 저장한다.
 */
...
/* 
 *  인터럽트 핸들러 안에 local_irq_disable(), local_irq_enable()가 
 *  존재한다면, 앞서 선언된 local_irq_disable()가 무효가 되므로, 
 *  현재상태 flag를 저장하고 (작업이 끝나면) 복구를 해줘야한다.
 */
...
/* interrupt flag를 가져와서 비교함 (enable(1), disalbe(0)) */
