...
/* __read_mostly : 이 데이터는 자주 수정되지 않으며 대부분 읽기 연산만 이루어진다. */
/* ops :설정된 scheduler */
...
static void schedule(void)
{
	...
	/* (1) CPU가 선점이 안되있고 (2) irq상태가 아니며 (3) irq가 가능한지 확인한다. */
	...
	/* runq 등 자료구조의 변경 등을 하는 do_schedule()함수를 lock을 걸어 보호 */
	...
	/* pre VCPU 상태 : blocked, runnable, offline  */
...
static int cpu_schedule_up(unsigned int cpu)
{
	 ...
	 *			 +-----------------------+
	 *   sd ---> | 						 |
	 *           | per_cpu_schedule_data | 
	 *           |						 |
	 *           +-----------------------+
	 *           | per_cpu_scheduler     | ---> ops
	 *           +-----------------------+
	 *		     |                       |
	 */
...
void __init scheduler_init(void)
{
    ...
    for ( i = 0; i < ARRAY_SIZE(schedulers); i++ )
    {
		...
		/* strcmp(s1,s2) : (같으면 0) */
		/* ops.name 이 존재하지 않고, opt_sched 와 scheduler[i]의 opt_name이 같은가? */
        else if ( !ops.name && !strcmp(schedulers[i]->opt_name, opt_sched) )
            ops = *schedulers[i];
    }

	/* opt_sched과 같은 opt_nmae을 가진  scheduler가 없으므로, 배열에 존재하는 값 대입 */ 
    if ( !ops.name )
    {
	...
	/*0번 CPU의 percpu 값에서 scheduler data와 type 초기화 */
	...
	/* Schdule Rate Limiting의 범위를 검사하여 문제시 기본값으로 설정함  */
