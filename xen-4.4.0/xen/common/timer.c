...
/*  timer의 상태에 따라,
 *  heap에 있다면 heap에서 제거하고, list에 있다면 list에서 제거한다. 
 *  timer의 상태를 TIMER_STATUS_invalid 로 만들어 준다.
 */
...
static inline bool_t timer_lock(struct timer *timer)
{
	...
	/* percpu의 timer 값과 timer가 일치할 때 timer.lock 값으로 spin_lock을 함 */
    for ( ; ; )
    {
...
/* 
 *  1. 현재의 상태 flags을 저장하고 irq_disable 한다. 
 *  2. timer에 lcok을 한다. 이것이 성공하면 __x = 1 , 
 *     실패하면 __x = 0, 저장된 상태 flags로 복구한다.
 */
...
{
    ASSERT(timer->status >= TIMER_STATUS_inactive); // >= 1
    ASSERT(timer->status <= TIMER_STATUS_in_list);  // <= 4 ( In use; on overflow linked list)
    return (timer->status >= TIMER_STATUS_in_heap); // >= 3 (In use; on timer heap)
}
