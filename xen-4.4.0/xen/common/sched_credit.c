...
/* 
 *  chsched_vcpu 구조체의 runq_elem->next가 가르키는 값이 무엇인가?
 *  자기자신 : runq에 존재하지 않는다.
 *  그 외 : runq에 존재한다.
 */
static inline int
...
__runq_insert(unsigned int cpu, struct csched_vcpu *svc)
{
	...
	/* cpu의 runqueue에 있는 csched_vcpu들과 우선순위를 비교하여 svc가 클 때 */
    ...
    /*
     *  만약 vcpu가 yield가 된 상태이고, 가능하면 
     *  우선순위가 낮고 실행가능한 vcpu의 하나 뒤에 해당 vcpu를 넣도록한다.
     *  queue가 너무 길어지면 30ms 안에 다음 runq_sort가 vcpu를 앞에 넣는다.
     *  이유 : ???
     */
...
__runq_tickle(unsigned int cpu, struct csched_vcpu *new)
{
    ...
    ASSERT(cur);
    cpumask_clear(&mask); /* clear all bits */
    idlers_empty = cpumask_empty(prv->idlers); /* Is mask empty (no bits sets) */
     ...
     */
	/*
	 * pcpu가 idle, 혹은 idler가 없거나(???) 새로운 vcpu가 기존 vcpu보다
	 * 높은 우선순위에 있다면, 여기서 실행된다.
	 *
	 * idle cpu가 있다면, 우선 new를 수행할 적합한 것을 찾아 기존의 것을 
	 * 선점하는 것을 피한다. new를 수행할 적합한 idler가 없다면, 여기서 실행된다.
	 * 그러나 cur을 수행하는 적합한 idler를 대신 찾는다.
	 */
    
	/* 1. cur이 idle 상태이거나 2. idler가 없고 new 의 우선순위가 cur보다 높다 */
         ...
         || (idlers_empty && new->pri > cur->pri) )
    {
        ...
        cpumask_set_cpu(cpu, &mask); /* turn on bit 'cpu' in mask */
    }
...
csched_vcpu_insert(const struct scheduler *ops, struct vcpu *vc)
{
	...
	/* svc가 런큐에 있지 않고 && vc가 runnable 상태이며 && vc가 running이 아니다  */
...
csched_vcpu_sleep(const struct scheduler *ops, struct vcpu *vc)
{
	...
	/* 현재 실행중인 VCPU면 schedule_softirq를 발생시키고, 아니면 runq에서 제거한다. */
...
csched_vcpu_wake(const struct scheduler *ops, struct vcpu *vc)
{
	...
	/*
     *  vcpu가 pcpu에서 수행 중 : ++percpu__perfcounters[PERFC__vcpu_wake_running] 
	 *  vcpu가 runq에 존재      : ++percpu__perfcounters[PERFC__vcpu_wake_onrunq]i
	 * 
	 *  (vcpu가 수행 중이지 않고 runq에도 존재하지 않을 때)
	 *  vpcu가 runnable : ++percpu__perfcounters[PERFC__vcpu_wake_runnable]
	 *  그 외의 경우    : ++percpu__perfcounters[PERFC__vcpu_wake_not_runnable] 
	 */
     ...
     */
	/*
	 * awaking VCPU들의 우선순위를 일시적으로 boost 한다.
	 *
	 * 만약 VCPU가 무시할 수 없을 정도로 CPU를 사용하면,
	 * 우선 순위를 정상으로 reset하는 credit 계산 코드를 찾아간다.
	 * 
	 * 만약 반대로 VCPU가 적은 CPU를 사용하고 (I/O 작업처럼)
	 * block과 awake를 반복하면 우선순위가 boost 된 상태로 유지된다.
	 * wake-to-run 응답시간을 최대한 활용하면서,
	 *
	 * wake-to-run latency에 민감한 VCPU가 CPU 자원에 집중적인 VCPU들을
	 * 선점할 수 있도록한다. 공평성에 영향을 주지 않으면서,
	 *
	 * ??? overspent한 credits을 얻은 후에 다시 시작하는 capped domains의
	 * VCPU에 대해서는 boost를 하지 않는 예외가 있다.
	 */
    ...
    /* Put the VCPU on the runq and tickle CPUs */
  	/* vcpu를 runq에 넣고, 우선 순위 등을 고려하여 PCPU에 할당을 결정한다. */ 
...
/*  sched_data에 csched_private 구조체 할당 
 *  active_sdom, master(UINT_MAX), 
 *  tslice_ms, ticsk_per_tslice, tick_period_us, credits_per_tslice
 *  ratelimit_us 초기화
 */
static int
